...
The API is not finalized and functions are half-implemented. Use with caution.
"""
...
def attributes(attrs):
    ...
    for k, v in attrs.items():
        ...
        ret += ' ' + k
        if v is True: continue  # Short attribute
        v = str(v)
        if not v.isalnum(): v = '"' + v.replace("&", "&amp;").replace('"', "&quot;") + '"'
        ret += '=' + v
    return ret
...
omit_endtag = {
    # Void elements:
    "area", "base", "br", "col", "emdeb", "hr", "img", "input", "keygen", "link",
...
class Builder:
    ...
    def _template(self):
        return lambda self=self: deepcopy(self)

    def __str__(self):
        ret = ""
        for n, (fhtml, builder) in self._fragments.items():
            ret += fhtml
            if builder: ret += builder.__html__()
        return HTML(ret + self._html + self._endtag)
    __html__ = _repr_html_ = __str__
    def __iter__(self): return str(self).__iter__()

    def __getattr__(self, name):
        """Names that don't begin with underscore are HTML tag names or template blocks."""
        if name[0] == "_": return object.__getattr__(self, name)
        self._html += self._endtag
        self._endtag = ""
        if name[0].isupper():
            if name in self._fragments: return self._fragments[name][1]
            self._fragments[name] = [self._html, Builder()]
            self._html = ""
            return self
        tagname = mangle(name)
        self._html += "<" + tagname + ">"
        if not tagname in omit_endtag: self._endtag = "</" + tagname + ">"
        return self

    def __setattr__(self, name, value):
        if not name[0].isupper(): return object.__setattr__(self, name, value)
    ...
    def _(self, *_content):
        ...
        for c in _content:
            ...
            assert c is not self, "Cannot add document to itself. Use E.elemname for subelements."
            if isinstance(c, Builder) and c._fragments:
                if self._html: self._fragments[id(self._html)] = [self._html, None]
...
def Document(*title, _urls = None, **html_attrs):
    ...
    if html_attrs: doc.html(**html_attrs)
    if title:
        doc.meta(charset="utf-8")  # Always a good idea
        doc.title(*title)
    for url in _urls or ():
        if url.endswith(".js"): doc.script(None, src=url)
        elif url.endswith(".css"): doc.link(rel="stylesheet", href=url)
        elif url.endswith(".ico"): doc.link(rel="icon", href=url, type="image/x-icon")
        elif url.endswith(".svg"): doc.link(rel="icon", href=url, type="image/svg+xml")
        elif url.endswith(".png"): doc.link(rel="icon", href=url, type="image/png")
        elif url.endswith(".webp"): doc.link(rel="icon", href=url, type="image/webp")
        elif url.endswith("manifest.json"): doc.link(rel="manifest", href=url)
